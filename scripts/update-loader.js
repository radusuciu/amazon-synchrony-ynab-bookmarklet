...
async function updateLoader(options = {}) {
  ...
  // Handle both formatted and minified versions
  const iifePart = loaderContent.match(
    /\(function\s*\(\)\s*{[\s\S]*?}\)\s*\(\);/
  );
  if (!iifePart) {
  ...
  fs.writeFileSync(
    path.join(distPath, "bookmarklet-loader.min.js"),
    bookmarkletCode
  );
  ...
  if (!silent) {
    console.log(
      "✅ Bookmarklet loader minified and saved to dist/bookmarklet-loader.min.js"
    );
  }
